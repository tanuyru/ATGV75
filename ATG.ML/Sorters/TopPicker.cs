...
{
    public class TopPicker : ISorter<RaceEntryModel>
    {
        public readonly int TopFavs;
        public TopPicker(int topDistToTake)
        {
            TopFavs = topDistToTake;
        }
        public IOrderedEnumerable<RaceEntryModel> Sort(IEnumerable<RaceEntryModel> entries, RaceModel race)
        {
            ...
            for (int i = 0; i < TopFavs && i < dist.Count; i++)
            {
                models.Add(dist[i]);
